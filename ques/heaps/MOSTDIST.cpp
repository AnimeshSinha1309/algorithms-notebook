...
int main() {
    ...
    priority_queue<point, vector<point>, decltype(compare)> ptPP(compare),
        delPP(compare), ptPM(compare), delPM(compare), ptMM(compare),
        delMM(compare), ptMP(compare), delMP(compare);
    while (q--) {
        char ch;
        point p(0, 0, 0);
        cin >> ch;
        switch (ch) {
            ...
            case '-':
                ...
                while (!ptPP.empty() && equals(ptPP.top(), delPP.top())) {
                    ptPP.pop();
                    delPP.pop();
                }
                while (!ptPM.empty() && equals(ptPM.top(), delPM.top())) {
                    ptPM.pop();
                    delPM.pop();
                }
                while (!ptMM.empty() && equals(ptMM.top(), delMM.top())) {
                    ptMM.pop();
                    delMM.pop();
                }
                while (!ptMP.empty() && equals(ptMP.top(), delMP.top())) {
                    ptMP.pop();
                    delMP.pop();
                }
                break;
            case '?':
                ...
                p.y ^= ans;
                ans =
                    MAX(MAX(distance(p, ptPP.top()), distance(p, ptPM.top())),
