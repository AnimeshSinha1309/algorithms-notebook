...
class Graph {
   public:
    ...
    struct Node { int index; vpl adjacent; NodeColor color = UNVISITED; };
    vector<Node> list; int n;
    Graph(int n) {
        list.resize(n);
    ...
    pair<vll, vll> dijkstra(vll from) {
        ...
        for (auto index : from) dist[index] = 0, q.emplace(0, index);
        while (!q.empty()) {
            pll top = q.top(); q.pop();
    ...
    vector<int> topological_sort() {
        ...
        for (auto node : list) for (auto route : node.adjacent)
                in_degree[route.first - 1]++;
        queue<int> process;
        for (int i = 0; i < list.size(); i++)
            if (in_degree[i] == 0) { process.push(i); result.push_back(i); }
        while (!process.empty()) {
            int processing = process.front(); process.pop();
    ...
    mll components() {
        vbl visited(n); mll result(0);
        for (int i = 0; i < n; i++) {
            if (visited[i]) continue;
            vll component; stack<ll> process;
            process.push(list[i].index); component.push_back(i); visited[i] = true;
            while (!process.empty()) {
                ll processing = process.top(); process.pop();
                for (pll neighbor : list[processing].adjacent)
                    if (!visited[neighbor.first])
                        process.push(neighbor.first), component.push_back(neighbor.first),
            ...
            for (int source = 0; source < n - 1; source++) {
                ...
                for (const auto &edge : list[source].adjacent) {
                    ...
                    if (distances[source] + edge.second < distances[sink])
                        distances[sink] = distances[source] + edge.second, parent[sink] = source;
                }
            }
        for (ll source = 0; source < n - 1; source++) // -ve cycle check
            for (const auto &edge : list[source].adjacent) {
                ll sink = edge.first;
                if (distances[source] + edge.second < distances[sink]) {
                    for (ll i : from) distances[i] = -1;
    ...
    mll floyd_warshall() {
        ...
        for (int i = 0; i < n; i++) for (auto route : list[i].adjacent) 
            distances[i][route.first] = route.second;
        for (int k = 0; k < n; k++) {
            for (int i = 0; i < n; i++) {
                for (int j = 0; j < n; j++) {
                    if (distances[i][k] == INT64_MAX || distances[k][j] == INT64_MAX) continue;
    ...
    pair<ll, vll> prims_mst() {
        ...
        vbl visited(n, false);
        for (int i = 0; i < n; i++) {
            if (!visited[i]) routes.emplace(INT32_MAX, i);
            while (!routes.empty()) {
                pll best = routes.top(); routes.pop();
                if (!visited[best.second]) costs[best.second] = best.first;
                visited[best.second] = false;
                for (const auto &path : list[best.second].adjacent)
                    if (!visited[path.second]) routes.push(path);
            }
        }
        return {accumulate(costs.begin(), costs.end(), 0), costs};
    }
};