...
public:
    ...
    explicit Node (Type bracket) {
        total = 0;
        if (bracket == OPEN) { open = 1; close = 0; }
        else if (bracket == CLOSE) { open = 0; close = 1; }
        else { open = 0; close = 0; }
    }
    static Node merge(Node a, Node b) {
        long long temp = min(a.open, b.close); Node c(EXPRESSION);
...
int main() {
    string st; cin >> st; vector<Node> list; for (char ch : st) list.emplace_back(ch == '(' ? Node::OPEN : Node::CLOSE);
    SegmentTree<Node> tree(list, Node::merge, *(new Node));
    int q; cin >> q; while(q--) { unsigned long l, r; cin >> l >> r; cout << tree.query(l-1, r).total * 2 << endl; }
}