...
struct DirectedGraph {
    ...
    DirectedGraph(int n) {
        size = n; order.resize(size); adjacent_f.resize(size);
    }
    void add_edge(int v1, int v2) {
        adjacent_f[v1].push_back(v2); adjacent_r[v2].push_back(v1);
    }
    void _scc_dfs1(int u) {
        visited[u] = 1;
        for (auto w : adjacent_f[u]) if (!visited[w]) _scc_dfs1(w);
        order.push_back(u);
    }
    void _scc_dfs2(int u) {
        visited[u] = 1; comp[u] = curr; comp_nodes[curr].push_back(u);
        for (auto w : adjacent_r[u]) if (!visited[w]) _scc_dfs2(w);
    }
    void stongly_connected_components() {
        fill(visited.begin(), visited.end(), false);
        order.clear();
        for (int i = 0; i < size; i++) if (!visited[i]) _scc_dfs1(i);
        fill(visited.begin(), visited.end(), false);
