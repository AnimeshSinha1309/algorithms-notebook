...
class AVLTree {
   ...
   private:
    ...
    Node *__insert_node(Node *T, Type x) {
        if (T == nullptr) {
            ...
            if (__height_difference(T) == -2)
                T = (x > T->right->data) ? __insert_right_right(T)
                                         : __insert_right_left(T);
        } else if (x < T->data) {
            T->left = __insert_node(T->left, x);
            if (__height_difference(T) == 2)
                T = (x < T->left->data) ? insert_left_left(T)
                                        : __insert_left_right(T);
        }
        T->height = __height(T);
    ...
    Node *__delete_node(Node *T, Type x) {
        Node *p;
        if (T == nullptr) {  // Delete nothing
            return nullptr;
        } else if (x > T->data) {  // Go to the right to find entity to delete
                                   // and balance
            T->right = __delete_node(T->right, x);
            if (__height_difference(T) == 2)
                T = (__height_difference(T->left) >= 0)
                        ? insert_left_left(T)
                        : __insert_left_right(T);
        } else if (x < T->data) {  // Go to the left to find entity to delete
                                   // and balance
            T->left = __delete_node(T->left, x);
            if (__height_difference(T) == -2)
                T = (__height_difference(T->right) <= 0)
                        ? __insert_right_right(T)
                        : __insert_right_left(T);
        } else {  // Delete the actual node
            if (T->right != nullptr) {
                p = T->right;
    ...
    Node *__search_quantile(Node *T, unsigned long count) {
        if (T == nullptr || count == 1) {
            ...
            return __search_quantile(T->right,
                                     count - __subtree_size(T->left) - 1);
        }
    }
   ...
   public:
    ...
    unsigned long count_eq(Type x) {
        return (root != nullptr ? root->subtree_size : 0) -
               __count_lt(root, x) - __count_gt(root, x);
    }
